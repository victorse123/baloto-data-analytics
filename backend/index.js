...
// =========================================================================
// ENDPOINT CONECTADO A SUPABASE PARA ALIMENTAR EL EMULADOR EN VIVO (DINÁMICO Y ALEATORIO)
// =========================================================================
app.get('/api/estadisticas-emulador', async (req, res) => {
  try {
    ...
    // 2. Para Frías: Ordenar de MENOR a MAYOR frecuencia
    ...
    // Función de mezcla Fisher-Yates optimizada
    ...
    // Aplicamos tu estrategia: primeros 10 en desorden
    const calientesAleatorios = desordenarArreglo(calientes.slice(0, 10));
    const friosAleatorios = desordenarArreglo(frios.slice(0, 10));
    
    // Las SuperBalotas se mezclan completas (son arreglos cortos de 4 elementos)
    const sbCalientesAleatorias = desordenarArreglo(sbCalientes);
    const sbFriasAleatorias = desordenarArreglo(sbFrias);

    // Enviamos el JSON con los datos mezclados dinámicamente
    res.json({ 
      calientes: calientesAleatorios, 
      frios: friosAleatorios, 
      sbCalientes: sbCalientesAleatorias, 
      sbFrias: sbFriasAleatorias 
    });
