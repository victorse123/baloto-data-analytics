...
function App() {
  // 1. Añadimos 'fecha' con un string vacío al estado inicial
  const [formData, setFormData] = useState({
    fecha: '',
    tipo: 'Tradicional',
    b1: '',
  ...
  const handleSubmit = async (e) => {
    ...
    // 2. Pasamos la fecha directo en el objeto que va hacia el backend
    const datosAEnviar = {
    ...
    try {
      ...
      if (respuesta.ok) {
        setMensaje(`✅ ${resultado.mensaje}`);
        setActualizarHistorial(prev => !prev);
        // Limpiamos el formulario (incluyendo la fecha)
        setFormData({ fecha: '', tipo: 'Tradicional', b1: '', b2: '', b3: '', b4: '', b5: '', sb: '' });
      } else {
        setError(`❌ Error: ${resultado.error}`);
      }
    } catch (err) {
  ...
  return (
    <div className="container" style={{ padding: '20px', maxWidth: '500px', margin: '0 auto' }}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        {/* NUEVO CAMPO: Calendario para la fecha del sorteo */}
        <div style={{ marginBottom: '15px' }}>
