...
export const EmuladorJuego = () => {
  ...
  // --- FUNCIÓN AUXILIAR: MEZCLADOR ALEATORIO (SHUFFLE) ---
  const desordenarArreglo = (arreglo) => {
    let copia = [...arreglo];
  ...
  const generarJugadaEstadistica = () => {
    ...
    setTimeout(() => {
      ...
      // 1. Clasificación base según la estrategia seleccionada
      if (estrategia === 'caliente') {
        poolBalotasBase = estadisticasReales.calientes || []; 
        poolSBBase = estadisticasReales.sbCalientes || [];
      } else if (estrategia === 'frio') {
        poolBalotasBase = estadisticasReales.frios || [];
        poolSBBase = estadisticasReales.sbFrias || [];
      } else {
        // Balanceado (Mezclado estructural de los primeros del top)
        poolBalotasBase = [
          ...(estadisticasReales.calientes || []).slice(0, 7), 
          ...(estadisticasReales.frios || []).slice(0, 7)
        ];
      ...
      // 2. APLICACIÓN DE ALEATORIEDAD: Desordenar antes de extraer
      // Mezclamos el pool completo que construimos para que en cada clic salgan combinaciones distintas
      const grupoBalotasMezclado = desordenarArreglo(poolBalotasBase);
      const grupoSBMezclado = desordenarArreglo(poolSBBase);

      // 3. Extracción estricta de 5 balotas únicas basadas en el set mezclado
      const balotasSeleccionadas = [];
      
      grupoBalotasMezclado.forEach(num => {
        const n = parseInt(num, 10);
        // Filtramos que sea un número válido entre 1 y 43 y que no esté ya repetido
        if (!isNaN(n) && n >= 1 && n <= 43 && !balotasSeleccionadas.includes(n) && balotasSeleccionadas.length < 5) {
          balotasSeleccionadas.push(n);
      ...
      // Ordenar de menor a mayor para la visualización estética en las esferas
      balotasSeleccionadas.sort((a, b) => a - b);

      // 4. Selección aleatoria y estricta de la Súper Balota del pool mezclado
      let sbSeleccionada;
      const poolSBLimpio = grupoSBMezclado.map(n => parseInt(n, 10)).filter(n => !isNaN(n) && n >= 1 && n <= 16);

      if (poolSBLimpio.length > 0 && !isNaN(poolSBLimpio[0])) {
        // Al estar mezclado el arreglo, el primer elemento [0] ya es aleatorio
        sbSeleccionada = poolSBLimpio[0];
      } else {
        // Si por alguna razón viene vacío, usamos una de respaldo seguro
        sbSeleccionada = estrategia === 'frio' ? 4 : 1; 
      ...
      // 5. Cargar los estados en React para pintar las esferas
      setCombinacionGenerada({
        balotas: balotasSeleccionadas,
        sb: sbSeleccionada
      });
